...
class MatMul4Bit(torch.autograd.Function):
    ...
    def forward(ctx, A, B, out=None, bias=None, quant_state: Optional[F.QuantState] = None):
        ...
        output = torch.nn.functional.linear(
            A, F.dequantize_4bit(B, quant_state).to(A.dtype, non_blocking=True).t(), bias
        )
    ...
    def backward(ctx, grad_output):
        ...
        if req_gradA:
            grad_A = torch.matmul(
                grad_output, F.dequantize_4bit(B, ctx.state).to(grad_output.dtype, non_blocking=True).t()
            )
